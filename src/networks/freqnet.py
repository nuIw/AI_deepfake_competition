...
def fft_fp32(x, dim=None, norm="ortho"):
    if not x.is_complex():
        y = torch.fft.fft(x.to(torch.float32), dim=dim, norm=norm)
        return y.to(torch.complex64)
    else:
        d = x.dtype
        y = torch.fft.fft(x.to(torch.complex64), dim=dim, norm=norm)
        return y.to(d)

def fft2_fp32(x, norm="ortho"):
    if not x.is_complex():
        y = torch.fft.fft2(x.to(torch.float32), norm=norm)
        return y.to(torch.complex64)
    else:
        d = x.dtype
        y = torch.fft.fft2(x.to(torch.complex64), norm=norm)
        return y.to(d)
...
class FreqNet(nn.Module):
    ...
    def hfreqWH(self, x, scale):
        ...
        x = fft2_fp32(x, norm="ortho")#,norm='forward'
    ...
    def hfreqC(self, x, scale):
        assert scale>2
        # print(f'input shape: {x.shape}, min: {x.min()}, max: {x.max()}, mean: {x.mean()}')
        x = fft_fp32(x, dim=1, norm="ortho")#,norm='forward'
